...
//50 is the maximum and the 1 is our minimum 
  ...
  if((x%2)==0)  {
   ...
   return Response.ok("EVA Hello World").build();     //200 code
  }
  else  {
   // odd
    ...
    if (x <= 10)  {    	
        ...
        return Response.status(500).entity("EVA ERROR CODE 500 RETURNED!!!").build();  
		...
		Thread.sleep(5000);
		return Response.status(503).entity("EVA ERROR CODE 503 RETURNED!!!").build();  
    }	 
    	...
    	System.out.println("--> response 204 sent!!!!");
    	 return Response.status(204).entity("EVA ERROR CODE 204 RETURNED!!!").build();  
    }
    else {
